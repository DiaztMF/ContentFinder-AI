...
export function Header({ onOpenIndexModal, savedCount = 0 }: HeaderProps) {
  ...
  return (
    <header className="sticky top-0 z-40 w-full border-b border-zinc-800 bg-[#09090b]/80 backdrop-blur-md transition-colors">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
        ...
        <Link href="/" className="flex items-center gap-3 group">
          ...
          <div className="flex items-center gap-2">
            <span className="font-bold text-lg sm:text-xl tracking-tight uppercase text-zinc-100">
              ContentFinder AI
            </span>
            <span className="text-[10px] font-bold px-1.5 py-0.5 rounded bg-blue-500/10 text-blue-400 border border-blue-500/20 uppercase tracking-widest hidden sm:inline-block">
              Engine
            </span>
          </div>
        </Link>
        ...
        <nav className="flex items-center gap-2 sm:gap-6">
          <Link
            ...
            className={`flex items-center gap-2 px-3 py-1.5 rounded-lg text-sm font-medium transition-all ${
              pathname === '/'
                ? 'text-zinc-100 font-semibold bg-zinc-900 border border-zinc-800'
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-4 py-2 bg-zinc-100 hover:bg-white text-zinc-950 rounded-md font-semibold text-xs tracking-wide transition-all cursor-pointer shadow-sm active:scale-[0.98]"
          >
